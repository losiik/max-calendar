...
## Назначение
Небольшой сервис запускает напоминания о встречах. Каждую минуту он обращается к backend ручке `/api/v1/reminder/`, которая проверяет предстоящие таймслоты и отправляет нотификации пользователям.
...
2. Создайте `.env` с переменной `BACKEND_URL`.  
