...
Для запуска в режиме разработчика - запустите `npm run dev`. Для сборки - `npm run build`. Для деплоя - `npm run deploy`.

## Переменные окружения
VITE_API_BASE_URL - базовый адрес бекенда (например `https://max.expalingpt.ru/api/v1`). ВАЖНО! Прежде чем использовать `http://localhost:9090/api/v1`, необходимо на бекенде отключить CORS в `/backend/main.py` или добавить необходимый хост. 
...
Если `init_data` недоступна (например, WebApp ещё не готов), фронт пробует снова при следующем запросе, чтобы всегда получить свежий токен и сообщить бэкенду о новом визите.
