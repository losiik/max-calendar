...
1. Прежде чем копировать репозиторий, необходимо настроить nginx и certbot.
   ```bash
      server {
         server_name <SERVER_NAME>;  # замените на ваше доменное имя или IP
         ...
         ssl_certificate_key /etc/letsencrypt/live/<SERVER_NAME>u/privkey.pem; # managed by Certbot
...
3. Далее запускаем `sudo certbot --nginx -d <SERVER_NAME> -d www.<SERVER_NAME>`
